...
*But!* What you can do is: subclass them in a new class, through inheritance, not giving it a `class_name`, and use _this_ node as an autoload. If it's a scene inheritance (with `.tscn` files), use the `New Inherited Scene...` menu from Godot, and if needed, don't forget to replace its script with a inherited script, using an `extends` declaration.

Here is a quick example for the `SxGameData` node (not a scene), useful for most projects (_state handling & persistable key-value store_).
...
Now a second example, using a scene: `SxSceneTransitioner`, useful to fade between game screens.

1. Create a *New Inherited Scene*, and open the node you want to inherit (here `SxSceneTransitioner`, located in `nodes/utils/SxSceneTransitioner/SxSceneTransitioner.tscn`).
2. Save it to a local folder (if you follow my advice you can put it in a top-level `autoloads` folder), with another name like `GameSceneTransitioner.tscn`.
3. In Godot, in the menu:Project settings[Autoloads] menu, add your `GameSceneTransitioner.tscn` scene as an autoload, with the `Global variable` checkbox enabled.
4. And that's it! In your game code, you can refer to `GameSceneTransitioner` to access your autoload!
****
...
* *<<_sxfabutton>>*: Ready-to-use `Button` with an integrated *FA* icon.
* *<<_sxfafont>>*: Font management wrapper around *FA*, useful for specific needs.
* *<<_sxfalabel>>*: A label which display a *FA* icon.

[WARNING]
.Don't forget your JSON!
====
When using FontAwesome components (any of the `SxFA` components), you need to make sure
you export `.json` files, because FontAwesome icon definitions are parsed at runtime.

In the menu:Project[Export] menu, add/select your preset and in the menu:Resources[Filters to export non-resource files/folders] section, you need to declare `*.json`.
====
...
* *<<_sxaudiomultistreamplayer>>*: Audio player with multiple simultaneous voices.
* *<<_sxglobalaudiofxplayer>>*: Autoload-ready multi stream player.
* *<<_sxglobalmusicplayer>>*: Autoload-ready audio player, useful for music tracks.

==== Debug

Debug nodes.

* *<<_sxdebugpanel>>*: Useful global debug UI, showing multiple panels:
...
* *Screen effects*: Full-screen effects you can use as nodes.
    ** *<<_sxbetterblur>>*: A gaussian blur, compatible with GLES2.
    ** *<<_sxmotionblur>>*: A motion blur.
    ** *<<_sxshockwave>>*: An animated shockwave effect.
    ** *<<_sxvignette>>*: A vignette effect.
